...
class LLMGateway:
        ...
        for provider_index, provider in enumerate(provider_sequence):
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    output = self._normalize_output(payload)
    ...
    def _source_label(self, provider: str) -> str:
        ...
        if provider_name in ("bytedance", "anthropic_compatible"):
            return "Remote"
        return "Fallback"

    def _system_prompt(self) -> str:
        return (
            "你是审讯对话引擎。你必须且只能输出一个JSON对象，禁止输出任何其他文字。\n"
            '格式：{"thought":"你的内心想法","speech":"你说出口的一句话","anchors":"关键词"}\n'
            "规则：\n"
            "- speech必须是角色直接说出的一句对话，不是分析、不是计划、不是多轮问答\n"
            "- thought是内心独白，不要暴露给对方\n"
            "- 不要输出多轮对话，只输出当前这一轮你说的一句话\n"
            "- 不要输出总结、分析、策略规划等非对话内容\n"
            '示例：{"thought":"他的不在场证明有漏洞","speech":"你说你11点在家睡觉，但邻居听到你开车出去了，怎么解释？","anchors":"时间线矛盾"}'
        )
